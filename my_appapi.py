...
# Database functions
  ...
  def db_create_table(self,conn,table,cols,table_constraints=""):
    ...
    try:
      self.db_query(conn,sqlstr)
      #curr.executescript(sqlstr)
    except:
      self.log("Error creating table {}".format(table))
      raise
    #finally:
      #curr.close()
  ...
  ###########
  #
  # db_query - returns a list of dictionaries of the columns selected
  #
  #
  ...
  def db_select(self,conn,query):
    result,desc=self.db_query(conn,query)
    ret=[]
    r={}
    if len(result)==0:
  ...
  def db_insert_row(self,conn,table,data_dict):
    for row in data_dict:
      ...
      try:
        ...
        self.log("Error inserting row {} into {}".format(datadict,table))
        raise
      #finally:
        #curr.close()
